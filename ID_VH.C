...
//		MS_Quit ("VL_MungePic: Not divisable by 4!");
		MS_Quit ("VL_MungePic: Not divisible by 4!");
	...
	unsigned width,unsigned height, unsigned frames, boolean abortable)
{
	...
	do	// while (1)
	{
		...
		{
			//
			// {seperate} separate random value into x/y pair
			//
			asm	mov	ax,[WORD PTR rndval]
			asm	mov	dx,[WORD PTR rndval+2]
			asm	mov	bx,ax
			asm	dec	bl
			asm	mov	[BYTE PTR y],bl			// low 8 bits - 1 = y {xoordinate} coordinate
			asm	mov	bx,ax
			asm	mov	cx,dx
			asm	mov	[BYTE PTR x],ah			// next 9 bits = x {xoordinate} coordinate
