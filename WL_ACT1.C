...
{SPR_STAT_6,bo_alpo},			// {Bad} Dog food					"
{SPR_STAT_7,block},				// {Red} White pillar				"
//
// NEW PAGE
//
{SPR_STAT_8,block},				// Tree								spr2v
{SPR_STAT_9},					// Skeleton flat					"
{SPR_STAT_10,block},			// Sink								" (SOD:{gibs} Bloody Skulls on Spike)
...
{SPR_STAT_19},					// {Skeleton relax} Pile of Bones	"
{SPR_STAT_20,bo_key1},			// Key 1							"
{SPR_STAT_21,bo_key2},			// Key 2							"
{SPR_STAT_22,block},			// {stuff} Bed						" (SOD:{gibs} Bloody Cage w/Skulls)
{SPR_STAT_23},					// {stuff} Water Bowl				"
//
// NEW PAGE
//
{SPR_STAT_24,bo_food}, 			// {Good food} Chicken Dinner		spr4v
...
{SPR_STAT_31,bo_bible},			// {Bible} Treasure Chest			"
//
// NEW PAGE
//
{SPR_STAT_32,bo_crown},			// crown							spr5v
{SPR_STAT_33,bo_fullheal},		// one up							"
{SPR_STAT_34,bo_gibs},			// {gibs} Bloody Bones				"
{SPR_STAT_35,block},			// barrel							"
{SPR_STAT_36,block},			// well								"
{SPR_STAT_37,block},			// Empty well						"
{SPR_STAT_38,bo_gibs},			// {Gibs 2} Blood Puddle			"
{SPR_STAT_39,block},			// flag								"
//
// NEW PAGE
//
//#ifndef SPEAR
{SPR_STAT_40,block},			// Call Apogee						{spr7v} spr6v (SOD: {SPR_STAT_40},Red Light)
//#else
...
// NEW PAGE
//
{SPR_STAT_41},					// {junk} Bone debris 1				"
{SPR_STAT_42},					// {junk} Bone debris 2				"
{SPR_STAT_43},					// {junk} Bone debris 3				"
//#ifndef SPEAR
...
//
#ifdef SPEAR
{SPR_STAT_48,block},			// {marble} tan pillar				spr7
{SPR_STAT_49,bo_25clip},		// {bonus 25 clip} ammo box			"
...
//			(unsigned)actorat[tilex][tiley] = 1;		// consider it a blocking tile
			(unsigned)actorat[tilex][tiley] = 64;		// changed it so that static objects show up in the map as hex numbers instead of grey stone walls
		case	dressing:
			laststatobj->flags = 0;
			break;
...
Open doors {conect} connect two areas, so sounds will travel between them and sight
	will be checked when the player is in a connected area.

Areaconnect is incremented/decremented by each door. If >0 they connect

Every time a door opens or closes the areabyplayer matrix gets recalculated.
	An area is true if it connects with the player's current {spor} spot.
...
{
	memset (areabyplayer,0,sizeof(areabyplayer));
	if (player->areanumber < NUMAREAS)				// added this as step 1 of fixing Grey Stone 1 bug
		areabyplayer[player->areanumber] = true;
}
...
{
//	int			area1,area2;
	unsigned	area1,area2;		// step 2a1 of Grey Stone 1 bug fix
	unsigned	far	*map;
	long		position;
	...
	//
		...
		if (area1 < NUMAREAS && area2 < NUMAREAS)		// step 2a2 of Grey Stone 1 bug fix
		{
			areaconnect[area1][area2]++;
			areaconnect[area2][area1]++;

			if (player->areanumber < NUMAREAS)			// step 2a3 of Grey Stone 1 bug fix
				ConnectAreas ();
		}
...
//	int		area1,area2,move;
	unsigned	area1,area2;		// step 2b1 of Grey Stone 1 bug fix
	unsigned	far	*map;
	long		position;
	int			tilex,tiley;
...
//
	...
	//
		...
		if (area1 < NUMAREAS && area2 < NUMAREAS)		// step 2b2 of Grey Stone 1 bug fix
		{
		areaconnect[area1][area2]--;
		areaconnect[area2][area1]--;

			if (player->areanumber < NUMAREAS)			// step 2b3 of Grey Stone 1 bug fix
				ConnectAreas ();
		}
	}
...
void MovePWalls (void)
{
	...
	// block crossed into a new block
		...
		*(mapsegs[0]+farmapylookup[pwally]+pwallx) = player->areanumber+AREATILE;
		...
		//
//		if (pwallstate>256)
		if (pwallstate>=256)	// fixed pushwall moving 3 tiles instead of 2
		{
		//
