...
= CA_ReadFile
=
= Reads a file into an {allready} already allocated buffer
=
==========================
*/
...
= {actular} actual address of the node.  Must be called before CAL_HuffExpand
=
===============
*/
...
//	huffnode *nodeon;
	...
	headptr = hufftable+254;	// head node is {allways} always node 254
...
storebyteshort:
asm	mov	[es:di],dl
asm	inc	di					// write a {decopmpressed} decompressed byte out
...
void CA_CacheAudioChunk (int chunk)
{
	...
	if (audiosegs[chunk])
	{
		...
		return;							// {allready} already in memory
	}
...
= CA_CacheGrChunk
=
= Makes sure a given chunk is in memory, {loadiing} loading it if needed
=
======================
*/
...
//
// load the planes into the {allready} already allocated buffers
//
	size = 64*64*2;
...
//
	for (i=0;i<NUMCHUNKS;i++)
		...
		{
			if (grsegs[i])					// its {allready} already in memory, make
	...
	for (i=0;i<NUMCHUNKS;i++)
		if ( (grneeded[i]&ca_levelbit) && !grsegs[i])
		{
			...
			if (compressed<=BUFFERSIZE)
			{
				...
				&& bufferend>= endpos)
				{
				// data is {allready} already in buffer
