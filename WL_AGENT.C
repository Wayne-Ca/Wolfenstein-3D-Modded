...
//} attackinfo[4][14] =
} attackinfo[4][4] =				// changed this to free memory
...
void DrawFace (void)
{
	if (gamestate.health)
	{
//		#ifdef SPEAR	// changed this to put god mode sprites in Wolf3D
		if (godmode)
...
//	else if (!buttonheld[bt_use] && doornum & 0x80)
	else if (!buttonheld[bt_use] && (doornum & 0x80) && (doornum & 0xc0) != 0xc0)	// fixes pushwall bug
	{
		buttonheld[bt_use] = true;
...
//		&& (check->flags & FL_VISABLE)
		&& (check->flags & FL_VISIBLE)				// spelling correction
...
//			&& (check->flags & FL_VISABLE)
	...
	//
	// trace a line from player to {enemey} enemy
	//
