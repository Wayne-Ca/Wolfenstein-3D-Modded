...
=		screenx,transx,transy,screenheight: projected edge location and size
=
= Returns true if the tile is {withing} within getting distance
=
========================
*/
...
boolean CheckOptimizedVertTile (void)
{
	ytile = yintercept >> TILESHIFT;
	return !doorobjlist[tilemap[xtile-xtilestep][ytile] & ~0x80].vertical;
}

boolean CheckVertTile (void)
{
	int wallnum;

	wallnum = tilemap[xtile - xtilestep][ytile];
	ytile = yintercept >> TILESHIFT;

	return (((wallnum & 0x80) && (wallnum & 0xc0) != 0xc0) && !doorobjlist[wallnum & ~0x80].vertical);
}

boolean CheckOptimizedHorizTile (void)
{
	xtile = xintercept >> TILESHIFT;
	return doorobjlist[tilemap[xtile][ytile-ytilestep] & ~0x80].vertical;
}

boolean CheckHorizTile (void)
{
	int wallnum;

	wallnum = tilemap[xtile][ytile - ytilestep];
	xtile = xintercept >> TILESHIFT;
...
//	if (lastside==1 && lastintercept == xtile && lasttilehit == tilehit)
	if (lastside==1 && lastintercept == xtile && lasttilehit == tilehit && !CheckOptimizedVertTile())
	{
...
//	if (lastside==0 && lastintercept == ytile && lasttilehit == tilehit)
	if (lastside==0 && lastintercept == ytile && lasttilehit == tilehit && !CheckOptimizedHorizTile())
	{
		// in the same wall type as last time, so check for optimized draw
		if (texture == (unsigned)postsource)
		{
...
int	CalcRotate (objtype *ob)
{
	...
	if (ob->state->rotate == 2)				// 2 rotation pain frame
		return 4*(angle/(ANGLES/2));		// {seperated} separated by 3 (art layout...)
...
= DrawScaleds
=
= Draws all objects that are {visable} visible
=
=====================
*/

//#define MAXVISABLE	50
#define MAXVISIBLE	177		// changed how many objects are visible at once to correct for invisible enemies and corrected spelling error
...
//	int 		i,j,least,numvisable,height;
	int 		i,j,least,numvisible,height;
	memptr		shape;
...
//
	for (statptr = &statobjlist[0] ; statptr !=laststatobj ; statptr++)
	{
		...
		if (!visptr->viewheight)
			continue;							// {to} too close to the object

//		if (visptr < &vislist[MAXVISABLE-1])	// don't let it overflow
		if (visptr < &vislist[MAXVISIBLE-1])	// don't let it overflow
			visptr++;
	}
...
//	if (!numvisable)
	if (!numvisible)
		return;									// no {visable} visible objects
...
// this wouldn't need to be done except for my debugger/video {wierdness} weirdness
...
//
// follow the walls from there to the right, {drawwing} drawing as we go
//
	VGAClearScreen ();
