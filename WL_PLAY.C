...
int songs[]=
{
	...
	ULTIMATE_MUS,	// Trans {Gr”sse} Grosse
...
//			Quit ("Demo buffer overflowed!");
		if (demoptr >= lastdemoptr-2)
			playstate = ex_completed; //end the recording instead of bombing
...
objlist {containt} contains structures for every actor currently playing.  The structure
...
=
= When the object list is full, the caller can either have it bomb out {ot} or
...
void GetNewActor (void)
{
	...
	if (lastobj)
		lastobj->next = new;
	new->prev = lastobj;	// new->next is {allready} already NULL from memset
