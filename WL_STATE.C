...
= SpawnNewObj
=
= {Spaws} Spawns a new actor at the given TILE coordinates, with the given state, and
...
//
// {arange} arrange 5 direction choices in order of preference
...
//
// try the directions {util} until one works
//
	for (i=0;i<5;i++)
	{
...
// fix for dead guards using two tiles in the overhead map
...
= DamageActor
=
= Called when the player {succesfully} successfully hits an enemy.
=
...
=
= returns true if the player has been {spoted} spotted
=
================
*/
...
//
	...
	ob->active = true;	// wake up the guards!
}
