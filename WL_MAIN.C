...
#define VIEWGLOBAL		0x10000					// globals {visable} visible flush to wall
...
//
// {proejection} projection variables
//
fixed			focallength;
unsigned		screenofs;
int				viewwidth;
...
void ReadConfig(void)
{
	...
	//
		...
		//MainMenu[6].active=1;		// so Read This! is selected
		MainMenu[7].active=1;		// so Read This! is selected
		MainItems.curpos=0;
	}
	else
	{
...
// ANGLES is assumed to be {divisable} divisible by four
//
...
// added this for tile16 objects in the overhead map
	for (i = STARTTILE16; i < STARTTILE16 + NUMTILE16; i++)
	{
		CA_CacheGrChunk (i);
...
{
	viewwidth = width&~15;					// must be {divisable} divisible by 16
...
//			PlayDemo (LastDemo++%4);
		...
		{
			RecordDemo ();
			StartCPMusic(INTROSONG);	// I added this line to turn the music back on when done recording
		}
		else
			US_ControlPanel (0);
