...
//	VWB_Vlin (yl-1,yl+viewheight,xl+viewwidth,BORDERCOLOR-2);
}
...
//	VWB_Hlin (xl-1,xl+viewwidth,yl+viewheight,BORDERCOLOR-2);
	VWB_Vlin (yl-1,yl+viewheight,xl-1,0);
	VWB_Vlin (yl-1,yl+viewheight,xl+viewwidth,125);
//	VWB_Vlin (yl-1,yl+viewheight,xl+viewwidth,BORDERCOLOR-2);
	VWB_Plot (xl-1,yl+viewheight,124);
//	VWB_Plot (xl-1,yl+viewheight,BORDERCOLOR-3);
}
...
void FinishDemoRecord (void)
{
	...
	*(unsigned far *)demoptr = length;
	...
	VW_FadeIn(); // correct for no fade in when done recording
	CenterWindow(24,3);
	// added this to make sure the color of the dialog remains black on white
	fontnumber=0;
	SETFONTCOLOR(0,15);
	// ================
	PrintY+=6;
...
//	demoplayback = false;	// unnecessary
...
#ifdef SPEAR
		if (gamestate.mapon == 20)	// give them the key {allways} always
		{
			gamestate.keys |= 1;
			DrawKeys ();
		}
