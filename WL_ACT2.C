...
dirtype dirtable[9] = {northwest,north,northeast,west,nodir,
	...
	while (move)
	{
		...
		//
			...
			if (doorobjlist[-ob->distance-1].action != dr_open)
				return;
			ob->distance = TILEGLOBAL;	// go ahead, the door is now {opoen} open
		}
...
//	new->flags = ob->flags | FL_SHOOTABLE;
	new->flags = (ob->flags & ~FL_NONMARK) | FL_SHOOTABLE;	// fixes a bug where hitler appears in every tile he moved through in the overhead map
