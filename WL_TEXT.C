...
^E{[enter]}[ENTER]				End of layout (all pages)
^G<y>,<x>,<pic>{[enter]}[ENTER]	Draw a graphic and push margins
^P{[enter]}[ENTER]				start new page, must be the first chars in a layout
...
#endif
{
	...
	do
	{
		...
		case sc_DownArrow:
		case sc_PgDn:
		case sc_RightArrow:		// the text {allready} already points at next page
			if (pagenum<numpages)
			{
				newpage = true;
				#ifdef JAPAN
