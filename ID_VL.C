...
//
	...
	outportb (SC_INDEX,SC_MAPMASK);		// leave this set {throughought} throughout?
...
= VL_SetLineWidth
=
= Line {witdh} width is in WORDS, 40 words is normal width for vgaplanegr
=
====================
*/
...
= due to some {incompatabilities} incompatibilities
=
=================
*/
...
void VL_MaskedToScreen (byte far *source, int width, int height, int x, int y)
{
	...
	dest = MK_FP(SCREENSEG,bufferofs);
	...
	for (w = 0; w < width; w++)
	{
		...
		for (h = 0; h < height; h++)
		{
			color = source[(h * (width >> 2) + (w >> 2)) + (w & 3) * (width >> 2) * height];

			if (color != 0xff)
				*(dest + (ylookup[y + h] + ((x + w) >> 2))) = color;
		}
	}
}
